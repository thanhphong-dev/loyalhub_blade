...
function setNavActive() {
    let currentPath = window.location.pathname.split("/public/")[1] || "admin";
    ...
    sidemenuItems.forEach((menuItem) => {
        let menuPath =
            menuItem.getAttribute("href") !== "javascript:void(0);"
                ? new URL(menuItem.getAttribute("href"), window.location.origin)
                      .pathname.split("/public/")[1]
                : null;

        // Kiểm tra nếu đường dẫn hiện tại trùng khớp với đường dẫn của mục menu
        if (menuPath === currentPath) {
            // Đánh dấu mục menu là active
            menuItem.classList.add("active");

            // Đánh dấu các mục cha là active và mở submenu
            let parentLi = menuItem.closest("li.slide.has-sub");
            if (parentLi) {
                parentLi.classList.add("active", "open"); // Giữ active và mở menu

                // Thêm active vào phần a của mục cha
                let parentLink = parentLi.querySelector("a.side-menu__item");
                if (parentLink) {
                    parentLink.classList.add("active"); // Thêm active vào phần a của mục cha
                }

                let childMenu = parentLi.querySelector(".slide-menu");
                if (childMenu) {
                    childMenu.style.display = "block"; // Hiển thị submenu
                }
            }
        } else {
            // Xóa trạng thái active khỏi các mục không liên quan
            menuItem.classList.remove("active");
        }
    });

    // Xử lý đóng/mở menu khi click vào mục cha
    document.querySelectorAll("li.slide.has-sub > a").forEach((parentLink) => {
        parentLink.addEventListener("click", (event) => {
            event.preventDefault(); // Ngăn điều hướng mặc định
            let parentLi = parentLink.closest("li.slide.has-sub");
            let childMenu = parentLi.querySelector(".slide-menu");

            // Đóng các mục cha khác trước khi mở mục mới
            document.querySelectorAll("li.slide.has-sub.open").forEach((li) => {
                if (li !== parentLi) {
                    li.classList.remove("open");
                    let submenu = li.querySelector(".slide-menu");
                    if (submenu) {
    ...
    // Đảm bảo tất cả các mục cha chứa mục con active được mở
    document.querySelectorAll("li.slide.has-sub.active").forEach((activeSlide) => {
        let childMenu = activeSlide.querySelector(".slide-menu");
        if (childMenu) {
            childMenu.style.display = "block";
